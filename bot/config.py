...
class Config:
    def __init__(self):
        try:
            ...
            self.FFMPEG = config(
                "FFMPEG",
                default='ab-av1 auto-encode -i "{}" --svt tile-columns=0:tune=1 --keyint 321 --max-crf 70 --min-vmaf 90 --preset 4 --vfilter scale=-2:480 -o "{}"',
            )
            ...
            self.MUX_ARGS = config(
                "MUX_ARGS",
                default="-c:v copy -c:a libopus -b:a 32k -ac 2 -map_metadata 0",
            )
